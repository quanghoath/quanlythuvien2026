...
function Page() {
  ...
  const openAdd = () => {
    ...
    setForm({
      TenSach: "",
      MaTacGia: tacGia[0]?.MaTacGia ?? 0,
      MaTheLoai: theLoai[0]?.MaTheLoai ?? 0,
      NamXuatBan: new Date().getFullYear(),
      SoLuong: 1,
      SoLuongCon: 1,
    });
  ...
  const openEdit = (r: Sach) => {
    ...
    setForm({
      ...
      SoLuong: r.SoLuong,
      SoLuongCon: r.SoLuongCon,
    });
    setOpen(true);
  };

  const submit = () => {
    if (!form.TenSach.trim() || !form.MaTacGia || !form.MaTheLoai)
      return toast.error("Vui lòng điền đủ thông tin");
    if (form.SoLuong < 0 || form.SoLuongCon < 0 || form.SoLuongCon > form.SoLuong)
    ...
    if (editing) {
      updateSach(editing.MaSach, payload);
      ...
      <CrudPage
        ...
        columns={[
          { key: "id", header: "Mã", render: (r) => r.MaSach, className: "w-16" },
          {
            key: "t",
            header: "Tên sách",
            render: (r) => <span className="font-medium">{r.TenSach}</span>,
          },
          {
            key: "a",
            header: "Tác giả",
            render: (r) => tacGia.find((x) => x.MaTacGia === r.MaTacGia)?.TenTacGia ?? "—",
          },
          {
            key: "c",
            header: "Thể loại",
          ...
          { key: "y", header: "Năm XB", render: (r) => r.NamXuatBan ?? "—", className: "w-20" },
          {
            ...
            header: "Tổng / Còn",
            render: (r) => (
              <Badge variant={r.SoLuongCon === 0 ? "destructive" : "secondary"}>
                {r.SoLuong} / {r.SoLuongCon}
              </Badge>
            ),
            className: "w-32",
          },
      ...
      >
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2 space-y-2">
            ...
            <Input
              value={form.TenSach}
              onChange={(e) => setForm({ ...form, TenSach: e.target.value })}
              autoFocus
            />
          </div>
          <div className="space-y-2">
            <Label>Tác giả</Label>
            <Select
              value={String(form.MaTacGia)}
              onValueChange={(v) => setForm({ ...form, MaTacGia: Number(v) })}
            >
              <SelectTrigger>
              ...
              <SelectContent>
                {tacGia.map((t) => (
                  <SelectItem key={t.MaTacGia} value={String(t.MaTacGia)}>
                    {t.TenTacGia}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Select
              value={String(form.MaTheLoai)}
              onValueChange={(v) => setForm({ ...form, MaTheLoai: Number(v) })}
            >
              <SelectTrigger>
              ...
              <SelectContent>
                {theLoai.map((t) => (
                  <SelectItem key={t.MaTheLoai} value={String(t.MaTheLoai)}>
                    {t.TenTheLoai}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  NamXuatBan: e.target.value === "" ? "" : Number(e.target.value),
                })
              }
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              type="number"
              min={0}
              value={form.SoLuongCon}
              onChange={(e) => setForm({ ...form, SoLuongCon: Number(e.target.value) })}
            />
          </div>
        </div>
      </FormDialog>
